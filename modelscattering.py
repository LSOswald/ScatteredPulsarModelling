...
def writePDVtimeSeries(dspec, freqs, tspan, nch=1, src='FRB', ofn='timeseries.ascii'): 
    ...
    if dspec.shape[1] % fDecFactor > 0:
        print('WARNING: the number of frequency channels {} is not an integer multiple of the number of subband channels {}, {} frequency channels will be dropped'.format(dspec.shape[1], nch, dspec.shape[1] % fDecFactor))
    ...
    hdrStr = '# File: {} Src: {} Nsub: 1 Nch: {} Npol: 1 Nbin: {} RMS: {}'.format(src, src, nch, decdspec.shape[0], np.std(decdspec))
    ...
    for chIdx in np.arange(nch):
        ...
        hdrStr += '\n# MJD(mid): 56384.680540767529 Tsub: {} Freq: {} BW: {}'.format(tspan, decfreqs[chIdx], bw)
        
        #write ascii profile
        # nsub nch nbin stokesI
        # 0 1 436 62.855
        for tIdx in np.arange(decdspec.shape[0]):
            hdrStr += '\n0 {} {} {}'.format(chIdx, tIdx, decdspec[tIdx, chIdx])
...
def find_rms(data,nbins):
    ...
    return np.min(rms_loc) # IS THIS THE BEST WAY? DUTY CYCLE?
...
def find_halfpowerwidth(data, nbins):
    # Get a very rough estimate of fractional pulse width to get an idea of resolution needed.
    peak = np.max(data)
    halfpower = np.argwhere(data > peak/2)
    if len(halfpower) > 0.:
        halfpowerwidth = ((halfpower[-1] - halfpower[0])/nbins)[0]
    else:
        halfpowerwidth = 0.
    return halfpowerwidth
...
#        print npmeans[i]
        ...
        # profile = profile + npamps[i]*np.exp(-0.5*(x-npmeans[i]/npsigmas[i])**2)
    return x, profile
...
def tau_1D_fitter(data,nbins):
    ...
    model.set_param_hint('nbins', value=nbins, vary=False)
    model.set_param_hint('sigma', value=15, vary=True, min =1, max = nbins) # SETTING SIGMA TOO SMALL IS A PROBLEM WITH GENERATING PROFILES.
    model.set_param_hint('mu', value=binpeak, vary=True, min=0, max = nbins)
    model.set_param_hint('A',value=profile_peak, vary=True,min=0)
...
def produce_taufits(filepath,readdir,writedir,pulsarname,obsdate,catDM,passfaillog,meth='iso',pulseperiod=None,snr_cut=None,
                    verbose=True, plotparams=False, plotflux=False, savefigure=False):
        pulsar, nch, nbins,nsub, lm_rms, tsub, MJD = read_headerfull(filepath, readdir)
        ...
        print0 = "Pulsar name: {}".format(pulsar)
        print1 = "Number of freq. channels: {} \nFreq channels will be labelled 0 - {}".format(nch, nch-1)
        print2 = "Number of bins: {}".format(nbins)
        print3 = "RMS: {:.2f}".format(lm_rms)
        print4 = "Tsub: {:.2f} sec".format(tsub)
        for k in range(5):
        ...
        # MOVING THIS LATER POST SNRCUT
        # residuals = np.zeros((nch, nbins))
        # fittedmodel = np.zeros((nch, nbins))

        for i in range(nch):
            print("--------------------------------------------------------")
            print("Channel {}".format(i))
            """Read in (pdv) data""" 
            data, freqc, freqm = read_data(filepath,readdir,i,nbins)
            ...
            if i ==0:
                ...
                if verboseTag:
                    print('peak bin at lowest freq channel:{}'.format(peakbin))
            else:
                peakbin = peakbin
                shift = int(halfway - int(peakbin))
            data = np.roll(data,shift)
            if verboseTag:
                print("Rolling data by -{} bins".format(shift))
            ...
            if meth is None:
                ...
                try:
                    ...
                    # Append the pulsar to log file.
                    f=open(passfaillog, "a+")
                    f.write('{}, failed, {}, no fit.\n'.format(filepath, meth))
                    f.close()
                    raise RuntimeError('Model made no successful fit.')
            elif meth == 'iso':
                try:
                    result, noiselessmodel, besttau, taustd, bestparams, bestparams_std, redchi, corsig = tau_fitter(data,nbins,verbose=verboseTag)
                except ValueError:
            ...
            # MOVING THIS LATER POST SNRCUT
            # # Calculate model residuals ready to be saved.
            # residuals[i,:] = data-noiselessmodel
            # fittedmodel[i,:] = noiselessmodel

            comp_SNR_model = find_peaksnr(noiselessmodel,comp_rms)

            if verboseTag:
                print('Estimated SNR (from model peak and data rms): {:.2f}'.format(comp_SNR_model))

            # BUGFIX
            if any(x is None for x in bestparams_std) is True:
                # comp_SNR = 0.
                data = np.zeros(len(data))
                print('Fixing a bug where the model can throw None values for the fit which is a problem. Setting data to 0s so that such cases are automatically removed later.')
            elif any(x is None for x in bestparams) is True:
                # comp_SNR = 0.
                data = np.zeros(len(data))
                print('Fixing a bug where the model can throw None values for the fit which is a problem. Setting data to 0s so that such cases are automatically removed later.')
            else:
            ...
            print('Estimated SNR (from data peak and rms): {:.2f}'.format(comp_SNR))
            print('Channel Tau (ms): {:.2f} \pm {:.2f} ms'.format(besttau,taustd))
            ...
            # MOVING THIS LATER POST SNRCUT
            # # WRITING TO CSV.
            # fit_sigma, fit_mean, fit_amplitude, fit_dc = bestparams
            # fit_sigma_err, fit_mean_err, fit_amplitude_err, fit_dc_err = bestparams_std
            # outputfilename = '{}/PSR_scatterfit.csv'.format(writedir)
            # print('Writing output to {}'.format(outputfilename))
            # with open(outputfilename, 'a+', newline='') as file:
            #     writer = csv.writer(file)
            #     writer.writerow([pulsarname, obsdate, freqm, fit_mean, fit_mean_err, fit_sigma, fit_sigma_err, fit_amplitude, fit_amplitude_err, fit_dc, fit_dc_err, besttau*pbs, taustd*pbs, comp_SNR, redchi/np.power(comp_rms,2)])
            ########################################################################
            
            # # WRITE OUT IMPORTANT PARAMETERS.
            # print('bestparams:', bestparams)
            # halfpowerwidth = find_halfpowerwidth(data, nbins)
            # f=open("tauvalues.txt", "a+")
            # if os.path.exists("tauvalues.txt") and os.path.getsize("tauvalues.txt") > 0:
            #     f.write("{}, {}, {}, {}, {}, {}, {}, {} \n".format(pulsarname, obsdate, freqm, besttau*pbs, taustd*pbs, comp_SNR, redchi/np.power(comp_rms,2), halfpowerwidth))
            # else:
            #     f.write("PSRNAME, MJD, FREQ, TAU, TAUERR, SNR, REDCHISQ, W50 \n")
            #     f.write("{}, {}, {}, {}, {}, {}, {}, {} \n".format(pulsarname, obsdate, freqm, besttau*pbs, taustd*pbs, comp_SNR, redchi/np.power(comp_rms,2), halfpowerwidth))
            # f.close()
        ...
        # print("--------------------------------------------------------")
        # print("--------------------------------------------------------")
        # print('Saving model residuals')
        # pickle.dump(residuals, open("{}/{}_{}".format(writedir,filepath,'residuals'), "wb"))
        # # Save fittedmodel.
        # print("--------------------------------------------------------")
        # print("--------------------------------------------------------")
        # print('Saving model')
        # pickle.dump(fittedmodel, open("{}/{}_{}".format(writedir,filepath,'model'), "wb"))
        ...
        if zero_ch:
            print("\n")
            print("{} channels have all zeroes (channels(s):".format(len(zero_ch)), zero_ch,  ") and will be removed.")
            if verboseTag:
                print("All zero channels are assigned SNR of 0")

        SNRs_to_save = np.copy(np.array(comp_SNRs))
        channel_number = np.arange(len(freqmsMHz))
          
        if snr_cut: 
            print("Using SNR cutoff of {:.2f}".format(snr_cut))
            comp_SNRs = np.nan_to_num(comp_SNRs)
            (ind_lowSNR,) = np.where(np.array(comp_SNRs) < snr_cut)
            print("SNR cutoff will exclude {} channels (channel(s): {})".format(len(ind_lowSNR), ind_lowSNR))
            ...
            channel_number_highsnr = np.delete(channel_number,ind_lowSNR)
            #New:
            ...
            SNRs_to_save_highsnr = np.delete(SNRs_to_save,ind_lowSNR)
            ...
            channel_number_highsnr = np.delete(channel_number,zero_ch)
            # New:
            ...
            SNRs_to_save_highsnr = np.delete(SNRs_to_save,zero_ch)
            ...
            channel_number_highsnr = np.copy(channel_number)
            # New:
            ...
            SNRs_to_save_highsnr = np.copy(SNRs_to_save)
        ...
        print("Will plot and save fits for remaining {}/{} channels".format(npch, nch))

        
        # Write to csv the fit parameters and other important variables.
        fit_sigma, fit_mean, fit_amplitude, fit_dc = paramset_highsnr*pbs
        fit_sigma_err, fit_mean_err, fit_amplitude_err, fit_dc_err = paramsetstd_highsnr*pbs
        outputfilename = '{}/{}_freqvariables.csv'.format(writedir, filepath)
        print('Writing output to {}'.format(outputfilename))
        with open(outputfilename, 'w', newline='') as file: #a+
            writer = csv.writer(file)
            writer.writerow(['FILEPATH', 'JNAME', 'MJD', 'P0', 'DM', 'METHOD', 'NBIN', 'NCHAN', 'CHANNEL', 'FREQUENCY', 'MEAN', 'MEAN_ERROR', 'SIGMA', 'SIGMA_ERROR', 'AMPLITUDE', 'AMPLITUDE_ERROR', 'DC', 'DC_ERROR', 'TAU', 'TAU_ERROR', 'SNR', 'CHISQ'])
            for i in range(npch):
                writer.writerow([filepath, pulsarname, obsdate, pulseperiod, catDM, meth, nbins, npch, channel_number_highsnr[i], freqMHz_highsnr[i], fit_mean[i], fit_mean_err[i], fit_sigma[i], fit_sigma_err[i], fit_amplitude[i], fit_amplitude_err[i], fit_dc[i], fit_dc_err[i], taussec_highsnr[i], lmfitstdssec_highsnr[i], SNRs_to_save_highsnr[i], redchis_highsnr[i]/np.power(comp_rmss_highsnr[i],2)])
       ...
       #"""Plot 1: Pulse profiles and fits"""
        ...
        if npch > 0:
            ...
            resnormed = (resdata-resdata.mean())/resdata.std() # SAVE THIS

            # Save residuals.
            print("--------------------------------------------------------")
            print("--------------------------------------------------------")
            print('Saving model residuals')
            pickle.dump(resdata, open("{}/{}_{}".format(writedir,filepath,'residuals'), "wb"))
            # Save model.
            print("--------------------------------------------------------")
            print("--------------------------------------------------------")
            print('Saving model')
            pickle.dump(model_highsnr, open("{}/{}_{}".format(writedir,filepath,'model'), "wb"))
            print("--------------------------------------------------------")
            print("--------------------------------------------------------")
            ...
            for k in range(numplots):
                ...
                for i in range(np.min([int(numsubplots),int(plots_remaining)])):
                    ...
                    plt.plot(profilexaxis,model_highsnr[j+i],lw = 2.0, alpha = 0.85, label=r'$\tau: {:.2f} \pm{:.2f}$ {}'.format(taulabel[j+i], taulabelerr[j+i], taustring))
                    plt.title('{} at {:.1f} MHz'.format(pulsar, freqMHz_highsnr[j+i]))
                ...
                if savefigure == True:
                    figname = '{}/{}_{}_{}_{}.png'.format(writedir, os.path.basename(filepath),'fitted_profiles', meth, k)
                    plt.savefig(figname, dpi=200)
                    print("Saved figure {} in {}".format(figname, writedir))
                    if noshow == False:
                        pass
                        #plt.show()
            ...
            if verboseTag:
                for i in range(npch):
                    print("Channel {}".format(i))
                    print('Tau (ms): {:.2f}'.format(1000*taussec_highsnr[i]))
                    tau1GHz = tauatfreq(freqMHz_highsnr[i]/1000.,taussec_highsnr[i],1.0,4)
                    print('tau1GHz_alpha_4 (ms) ~ {:.4f}'.format(tau1GHz*1000))
            ...
            if plotparams==True:
                ...
                if len(freqMHz_highsnr) < 3:
                    # Append the pulsar to log file.
                    f=open(passfaillog, "a+")
                    f.write('{}, failed, {}, {} channels.\n'.format(filepath, meth, len(freqMHz_highsnr)))
                    f.close()                    
                ...
                # ################################################
                # # PRINT OUT DM AND ITS ERROR.
                # f=open("dmvalues.txt", "a+")
                # if os.path.exists("dmvalues.txt") and os.path.getsize("dmvalues.txt") > 0:
                #     f.write('{}, {}, {}, {} \n'.format(pulsarname, obsdate, DMval, DMvalstd))
                # else:
                #     f.write('PSRNAME, MJD, DM, DMERR \n')
                #     f.write('{}, {}, {}, {} \n'.format(pulsarname, obsdate, DMval, DMvalstd))
                # f.close()
                # ################################################
                ...
                plt.plot(freqMHz_highsnr,powout.best_fit*pbs,'-', alpha=alfval,label='pow = {:.2f}'.format(powout.best_values['exponent']))
                plt.plot(freqMHz_highsnr,quadout.best_fit*pbs,'-',alpha=alfval, label='quad: a,b = {:.3f},{:.3f}' .format(quadout.best_values['a'],quadout.best_values['b']))
                ...
                plt.plot(DMmodelfit,freqMHz_highsnr, '-', label=r'DM: ${:.3f} \pm {:.3f}$ $\rm{{cm}}^{{-3}} \rm{{pc}}$'.format(DMval,DMvalstd), alpha = alfval)
                ...
                if savefigure == True:
                    figname2 = '{}/{}_{}.png'.format(writedir, os.path.basename(filepath),'fitting_parameters')
                    plt.savefig(figname2, dpi=200)
                    print("Saved figure {} in {}".format(figname2, writedir))
                if noshow == False:
                    pass
                    #plt.show()
            ...
            elif plotparams==False:
                DMval = 'none'
                DMvalstd = 'none'
        
        return freqMHz_highsnr, taussec_highsnr, lmfitstdssec_highsnr, DMval, DMvalstd, npch


def produce_tauspectrum(freqMHz,taus,tauserr,pulsarname,obsdate,writedir,log=True, plotspecevo=False,
                        savefigure=False):
    ...
    if len(taus) < 2:
        ...
        # # PRINT OUT ALPHA AND ITS ERROR.
        # f=open("alphavalues.txt", "a+")
        # if os.path.exists("alphavalues.txt") and os.path.getsize("alphavalues.txt") > 0:
        #     f.write('{}, {}, {}, {} \n'.format(pulsarname, obsdate, alpha, alphaerr))
        # else:
        #     f.write('PSRNAME, MJD, ALPHA, ALPHAERR \n')
        #     f.write('{}, {}, {}, {} \n'.format(pulsarname, obsdate, alpha, alphaerr))
        # f.close()

        plt.figure(figsize=(10,6))         
        plt.errorbar(freqMHz,taus,yerr=tauserr,fmt='x', markersize=10.0,capthick=2,linewidth=1.5, label='data')
        plt.plot(freqMHz,fit,ls='dashed',linewidth=1.5,label=r'$\alpha = {:.2f} \pm {:.2f}$'.format(alpha,alphaerr))
        if log:
            plt.xscale('log')
            plt.yscale('log')
        ...
        # BOOKMARK INCREASING FONT SIZES
        plt.yticks(fontsize=20)
        plt.xticks(fontsize=20)
        leg = plt.legend(fontsize=20,numpoints=None)
        plt.xlabel(r'$\nu$ (MHz)',fontsize=22, labelpad=15.0)
        plt.ylabel(r'$\tau$ (sec)',fontsize=22)

        plt.xlim(xmin = 0.95*freqMHz[0],xmax=1.05*freqMHz[-1])
        plt.gcf().subplots_adjust(bottom=0.15, top=0.97, right=0.96, left=0.11)
        
        if savefigure == True:
                figname = '{}/{}_{}.png'.format(writedir, os.path.basename(filepath),'tau_spectrum')
                plt.savefig(figname, dpi=200)
                print("Saved figure {} in {}".format(figname, writedir))
        if noshow == False:
            pass
        ...
        if plotspecevo == True:
            """Calculate spectral index evolution with loss of higher freq. channels"""
            npch = len(freqMHz)
            spec_sec = np.zeros(npch-1)
            spec_std_sec = np.zeros(npch-1)
                         
            for i in range(2,npch+1): #npch -1
            ...
            plt.figure(figsize=(10,6))
            plt.errorbar(freq_incl,spec_sec,yerr=spec_std_sec, fmt='x',alpha=0.6,markersize=12.0,capthick=2,linewidth=0.5)                    
            plt.title(r'Change in spectral index', fontsize=12)
            for x,y in zip(freq_incl[0::2], spec_sec[0::2]):
                plt.annotate('{:.2f}'.format(y), xy=(x,1.08*y), xycoords='data',textcoords='data')
            plt.ylim(plt.ylim()[0],1.1*plt.ylim()[1])
            plt.yticks(fontsize=12)
            plt.xticks(fontsize=12)
            plt.xlabel(r'Highest freq included (MHz)',fontsize=12)
            plt.ylabel(r'$\alpha$',fontsize=12)

            if savefigure == True:
                figname = '{}/{}_{}.png'.format(writedir, os.path.basename(filepath),'alpha_evolution')
                plt.savefig(figname, dpi=200)
                print("Saved figure {} in {}".format(figname, writedir))
            if noshow == False:
                pass
...
def produce_tauspectrum_highHBA(freqMHz,taus,tauserr,freqMHzhigh,tauhigh,tauerrhigh):
    ...
    plt.errorbar(freqMHz,taus,yerr=tauserr,fmt='*-',markersize=10.0,capthick=2,linewidth=1.5,label=r'$\alpha = {:.1f} \pm {:.1f}$'.format(alpha,alphaerr))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--readdir', default='.', type=str, help='Location where input is read from.')
    ...
    parser.add_argument('-passfaillog','--passfaillog', default='defaultlog.log', type=str, help="Path to log file where you save whether the program has passed or failed.")
    ...
    if snrcut:
        snrcut = float(snrcut)
    # else:
    #     snrcut = -1. # HACKY WAY OF REMOVING CHANNELS WITH SNR ARTIFICIALLY SET TO 0.
    ...
    pulsar, nch, nbins,nsub, lm_rms, tsub, MJD = read_headerfull(filepath, args.readdir)

    # SPECIFY OBS DATE AND TIME AS THE MJD READ FROM THE HEADER.
    obsdate = MJD #filepath.split('_')[1]
    ...
    if pulseperiod == None:
        ...
        command = ['psrcat','-nohead', '-o', 'short', '-c', 'JNAME p0 DM', pulsar] # HAD TO SPECIFY LOCATION OF PSRCAT BECAUSE I CAN'T GET IT ON THE PATH # /Data/loswald/psrcat/psrcat
        ...
        if return_code != 0:
            raise RuntimeError('Problems with psrcat: {}').format(stdoutdata)
        ...
        catDM = float(stdoutdata.split()[3])
        
        if pname != pulsar:
            raise RuntimeError("Pulsar name mismatch")
        print("{}. Period: {:.3f} sec, DM: {:.3f} pc cm^-3".format(pname, per,catDM))
    ...
    freqMHz, taussec, taustdssec, DMval, DMvalstd, npch = produce_taufits(filepath, args.readdir, args.writedir, pname, obsdate, catDM, args.passfaillog, meth=meth, pulseperiod=per, snr_cut = snrcut, verbose=True, plotparams=True, savefigure=savefig) # THERE IS A BUG THAT THIS CODE ONLY WORKS WITH MULTIPLE FREQUENCIES UNLESS YOU TURN PLOTTING OFF MANUALLY...
    
    """Produce tauspectrum"""
    freqMHz, alpha, alphaerr, fit = produce_tauspectrum(freqMHz, taussec,
                                                        taustdssec, pname, obsdate, args.writedir, log=True,
                                                        plotspecevo=True, savefigure=savefig)



    ########################################################################
    # Write alpha and DM correction to csv.
    alphafilename = '{}/{}_alphaDM.csv'.format(args.writedir, filepath)
    print('Writing alpha and DM to {}'.format(alphafilename))
    with open(alphafilename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['FILEPATH', 'JNAME', 'MJD', 'P0', 'DM', 'METHOD', 'NBIN', 'NCHAN', 'ALPHA', 'ALPHA_ERROR', 'DMCORR', 'DMCORR_ERROR'])
        writer.writerow([filepath, pname, obsdate, per, catDM, meth, nbins, npch, alpha, alphaerr, DMval, DMvalstd])
    ########################################################################
